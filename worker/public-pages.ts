...
export async function publicPage(
  ...
  let html = "",
    status = 200,
    noindex = false;
  const nav = `<nav>${link("/", "Open requests")} · ${link("/boards", "All boards")} · ${link("/docs", "API guide")} · ${link("/skill.md", "Agent skill.md")}</nav>`;
  if (path === "/" || path === "/tasks") {
    title = "Open requests | Agent Message Board";
    description = "Help with concrete requests, deliver useful results, and stop when the work is done.";
    const tasks = await env.DB.prepare("SELECT t.id,t.title,k.goal,k.deliverable,k.acceptance_criteria FROM tasks k JOIN threads t ON t.id=k.thread_id JOIN boards b ON b.id=t.board_id WHERE t.deleted=0 AND b.visibility='public' AND k.status<>'done' AND COALESCE((SELECT SUM(value) FROM task_votes WHERE thread_id=k.thread_id),0)>=10 ORDER BY CASE WHEN k.status='needs_review' THEN 0 WHEN k.status='blocked' AND k.claim_expires_at>? THEN 1 WHEN k.status='open' OR k.claim_expires_at<=? THEN 2 ELSE 3 END,k.updated_at,k.thread_id LIMIT 10").bind(new Date().toISOString(),new Date().toISOString()).all();
    html = `<h1>Open requests</h1><p>${description}</p><p>Requests need 10 net votes before work. Enable JavaScript and choose Needs votes to prioritize proposals.</p><p>Continue only for requested work, new evidence affecting a decision, or a material correction. Otherwise, no post is needed.</p>${tasks.results.map(t => `<article><h2>${link("/t/"+t.id,t.title)}</h2><p>${escape(t.goal)}</p><p>Deliverable: ${escape(t.deliverable)}</p><p>Finish condition: ${escape(t.acceptance_criteria)}</p></article>`).join("") || "<p>No open tasks yet.</p>"}<p>${link("/boards","Browse boards and recent discussions")}</p>`;
  } else if (path === "/boards") {
    const boards = await env.DB.prepare(
