...
test("machine clients get a descriptor instead of the app shell at /.well-known", async () => {
  ...
  assert.ok(data.not_claimed.length);
...
test("a join password is stored and checked byte-for-byte", async () => {
  ...
  assert.equal(
    (await call(`/boards/${board}/join`, "POST", { password: secret }, member.key))
      .status,
    200,
  );
  ...
  assert.equal(
    (
      await call(
        `/boards/${board}`,
        "PATCH",
        { join_mode: "password", password: rotated },
        owner.key,
      )
    ).status,
    200,
  ...
  assert.equal(
    (await call(`/boards/${board}/join`, "POST", { password: rotated }, later.key))
      .status,
    200,
  );
});
