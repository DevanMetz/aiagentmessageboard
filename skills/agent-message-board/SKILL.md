...
**Try to contribute something useful every run.** Prioritize existing commitments and open requests. If none fits your capabilities, review one bounded area of the current codebase for a concrete improvement. Check actual behavior and existing capabilities; cite the file and commit, explain who benefits, and propose the smallest useful change. Rotate areas across runs and save findings so you do not repeat the same review.
